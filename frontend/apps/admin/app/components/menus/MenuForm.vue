...
async function loadMenuTree() {
  try {
    const data = await fetchAdminGet<Array<Record<string, unknown>>>('/api/v1/admin/menus/tree', $api)
    ...
    function flatten(nodes: Array<Record<string, unknown>>, depth = 0) {
      for (const node of nodes) {
        ...
        items.push({ label: `${prefix}${node.label}`, value: node.id as string | number })
...
<template>
  <UForm ref="formRef" :schema="schema" :state="state" class="space-y-4">
    <UFormField label="菜单名称" name="label" required>
      <UInput v-model="state.label" placeholder="请输入菜单名称" class="w-full" />
    </UFormField>

    <UFormField label="权限标识" name="permission">
      <UInput v-model="state.permission" placeholder="如：system:menu:list" class="w-full" />
    </UFormField>
    ...
    <UFormField label="路由链接" name="href">
      <UInput v-model="state.href" placeholder="如：/systems/menus" class="w-full" />
    </UFormField>

    <UFormField label="菜单类型" name="menuType">
      <div class="flex gap-4">
        <label v-for="opt in menuTypeOptions" :key="opt.value" class="flex items-center gap-1.5 cursor-pointer">
          <input
            type="radio"
            :value="opt.value"
    ...
    <UFormField label="上级菜单" name="parentId">
      ...
      >
        ...
        <option v-for="item in parentMenuItems" :key="item.value" :value="item.value">
          {{ item.label }}
        </option>
      </select>
    </UFormField>
