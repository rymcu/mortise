...
export function useProfile() {
  ...
  const profile = useState<MemberProfile | null>('web-profile', () => null)
  const loading = useState('web-profile-loading', () => false)
  const error = useState('web-profile-error', () => '')
  ...
  async function fetchProfile(): Promise<MemberProfile | null> {
    ...
    error.value = ''
    try {
      const res = await $api<{ code: number; data: MemberProfile }>(
  ...
  async function updateProfile(data: {
    ...
    error.value = ''
    try {
      await $api('/api/v1/app/auth/profile', {
        method: 'PUT',
        body: data
      })
      // 同步刷新用户会话信息
