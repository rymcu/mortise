...
export function useMemberSecurity() {
  ...
  const sessions = useState<MemberClientSession[]>('standalone-member-client-sessions', () => [])
  const loading = useState('standalone-member-security-loading', () => false)
  const error = useState('standalone-member-security-error', () => '')
