...
1. 第一步：仓库装配
    ...
    - 状态：主仓库装配已完成；独立仓库推送与 gitlink 切换待执行。

2. 第二步：AIoT 核心闭环
    - 目标：设备注册、激活、JWT、心跳、影子链路可直接联调。
    - 状态：已完成第一版可落库实现，并补齐遥测上报、命令拉取与指令回执。
    - 待补充：设备冻结/退役等运营接口，以及 MQTT broker 真实推送联调。
...
5. 第五步：联调与回归
    - 目标：补接口测试、设备模拟调用脚本、真实 MQTT broker 集成。
    - 状态：当前仅完成编译级验证。
...
```text
mortise-aiot-gateway-mqtt/       ← MQTT 协议适配，第一阶段即引入
├── pom.xml
...
mortise-aiot-gateway-mqtt
  ...
  ├── mortise-aiot-infra           （CacheService、Mapper 等）
  ├── eclipse-paho-client / hivemq-mqtt-client
  └── mortise-cache                （可选，用于设备-连接映射）
```

**装配方式：** `mortise-aiot-gateway-mqtt` 作为独立 git submodule 仓库，通过根 pom `pro` Profile 纳入构建，由 `mortise-app` 的 `pro` Profile 装配。第一阶段即引入，与 AIoT/OTA 核心模块同步落地。
...
| 第一阶段 | MQTT | `mortise-aiot-gateway-mqtt` 独立 git submodule，与核心模块同步落地 |
...
建议在 `application.yml` 中增加以下配置命名空间：

```yaml
mortise:
  aiot:
    # 设备心跳超时（秒），超过此时间未心跳视为离线
    heartbeat-timeout: 180
    # 设备令牌有效期（秒）
    device-token-ttl: 3600
    # 设备令牌刷新窗口（秒），到期前多久允许刷新
    device-token-refresh-window: 600
    # 指令默认超时（秒）
    command-default-timeout: 60
    # 影子同步延迟容忍（秒）
    shadow-sync-delay-tolerance: 5
    # 遥测快照缓存 TTL（秒）
    telemetry-cache-ttl: 600
  ota:
    # 固件下载票据有效期（秒）
    download-ticket-ttl: 600
    # 单批次默认并发限制
    default-concurrency-limit: 100
    # 单任务默认超时（秒）
    default-task-timeout: 1800
    # 默认重试次数
    default-retry-limit: 3
    # 默认重试间隔（秒）
    default-retry-interval: 300
    # 批次失败阈值（百分比），超过后自动暂停
    failure-threshold-percent: 10
```
...
- 第一阶段同时落地 HTTP + MQTT 双协议接入，再逐步补齐灰度、回滚、更多协议和分析能力。
