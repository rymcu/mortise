...
```
第 6 层: mortise-app (应用层)
         └── 聚合所有模块

第 5 层: 业务域 API
         ├── mortise-system-admin / mortise-system-api
         └── mortise-member-admin / mortise-member-api

第 4 层: 业务域应用与基础设施
         ├── mortise-system-application / mortise-system-infra
         └── mortise-member-application / mortise-member-infra

第 3 层: 应用基础层
         ├── mortise-auth, mortise-web-support, mortise-monitor
         └── 同级模块，相互不依赖

第 2 层: 基础设施层
         └── mortise-log, mortise-cache, mortise-notification, mortise-persistence

第 1 层: 核心层
         └── mortise-common, mortise-core
```
...
mortise-web-support → mortise-core ✓
```

❌ **错误**:
```
mortise-monitor → mortise-auth ✗  (同层依赖)
mortise-auth → mortise-web-support ✗  (同层依赖)
mortise-web-support → mortise-monitor ✗  (同层依赖)
```
