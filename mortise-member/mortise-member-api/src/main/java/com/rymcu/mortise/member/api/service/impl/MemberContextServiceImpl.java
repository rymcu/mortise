...
@Slf4j
@Service
public class MemberContextServiceImpl implements MemberContextService {
    ...
    public Member getCurrentMember() {
        ...
        if (principal instanceof MemberDetailInfo userDetails) {
            Member member = userDetails.getMember();
            if (member != null) {
                return member;
            }
            log.error("MemberDetailInfo 中的 Member 实体为 null");
            throw new UsernameNotFoundException("用户信息不完整");
        }

        log.error("Principal 类型不是 MemberDetailInfo: {}", principal.getClass().getName());
        log.error("请确保 JWT 过滤器将 MemberDetailInfo 设置到 SecurityContext");
    ...
    public Optional<Member> getCurrentMemberOptional() {
        try {
            return Optional.of(getCurrentMember());
    ...
    public Long getCurrentMemberId() {
        return getCurrentMemberOptional()
                .map(Member::getId)
                .orElse(null);
    }
    ...
    public String getCurrentUsername() {
        ...
        if (principal instanceof MemberDetailInfo) {
            return ((MemberDetailInfo) principal).getUsername();
        }
    ...
    @Override
    public MemberDetailInfo getCurrentMemberDetails() {
        Authentication authentication = getAuthentication();
        ...
        if (principal instanceof MemberDetailInfo) {
            return (MemberDetailInfo) principal;
        }
