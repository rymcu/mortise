...
SELECT 25000000000000001, '会员', 'member', 'i-lucide-users', '/members', 0, 0, 1, 3, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
...
SELECT 25000000000000011, '查询会员', 'member:list', 'i-lucide-list', '', 0, 0, 2, 1, parent.id, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
...
INSERT INTO mortise.mortise_menu (id, label, permission, icon, href, status, del_flag, menu_type, sort_no, parent_id, created_time, updated_time)
SELECT 25000000000000012, '查看会员', 'member:query', 'i-lucide-eye', '', 0, 0, 2, 2, parent.id, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
FROM mortise.mortise_menu parent WHERE parent.permission = 'member'
AND NOT EXISTS (SELECT 1 FROM mortise.mortise_menu WHERE permission = 'member:query');

INSERT INTO mortise.mortise_menu (id, label, permission, icon, href, status, del_flag, menu_type, sort_no, parent_id, created_time, updated_time)
SELECT 25000000000000013, '编辑会员', 'member:edit', 'i-lucide-pencil', '', 0, 0, 2, 3, parent.id, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
FROM mortise.mortise_menu parent WHERE parent.permission = 'member'
