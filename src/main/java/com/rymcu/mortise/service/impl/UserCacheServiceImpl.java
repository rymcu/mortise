...
public class UserCacheServiceImpl {
     ...
     * @return 用户信息
     */
    @Cacheable(value = CacheConstant.USER_INFO_CACHE, key = "#userId", unless = "#result == null")
     ...
     * @return 用户会话信息
     */
    @Cacheable(value = CacheConstant.USER_SESSION_CACHE, key = "#userId", unless = "#result == null")
     ...
     * @return 权限列表
     */
    @Cacheable(value = CacheConstant.USER_PERMISSIONS_CACHE, key = "#userId", unless = "#result == null")
     ...
     * 当进行批量用户操作时调用
     */
    @CacheEvict(value = CacheConstant.USER_INFO_CACHE, allEntries = true)
    ...
    public void cacheServiceExample(Long userId) {
        // 使用通用缓存服务存储临时数据
        cacheService.putString(CacheConstant.TEMP_DATA_CACHE, "temp:" + userId, "临时数据");
        
        // 获取临时数据
        String tempData = cacheService.getString(CacheConstant.TEMP_DATA_CACHE, "temp:" + userId);
        log.info("获取到临时数据: {}", tempData);
        
        // 存储对象
        User user = userService.getById(userId);
        if (user != null) {
            cacheService.putObject(CacheConstant.HOT_DATA_CACHE, "user:" + userId, user);
            
            // 获取对象
            User cachedUser = cacheService.getObject(CacheConstant.HOT_DATA_CACHE, "user:" + userId, User.class);
