...
public class VoiceCatalogBootstrapRunner implements ApplicationRunner {
    ...
    private VoiceProvider loadProviderByCode(String code) {
        return voiceProviderRepository.findByCode(code)
                .orElseThrow(() -> new ServiceException("系统语音 Provider 同步后未找到记录: " + code));
    }

    private VoiceModel loadModelByCode(String code) {
        return voiceModelRepository.findByCode(code)
                .orElseThrow(() -> new ServiceException("系统语音模型同步后未找到记录: " + code));
    }

    private VoiceProfile loadProfileByCode(String code) {
        return voiceProfileRepository.findByCode(code)
                .orElseThrow(() -> new ServiceException("系统语音配置同步后未找到记录: " + code));
    }

    private void requireSuccess(Boolean result, String message) {
